...
def _run_dsessvcm_project():
    ...
    dsessvcm = DSESSVCM(\
        ...
        crossvalidation = sklearn_cv, 
        selfadaption = Selfadaption(),
        repair_method = 'project')

    dsessvcm.run()
    return dsessvcm

def _run_dsessvcm_mirror():
    ''' no documentation yet '''
...
def _run_dses():
    ...
    dses = DSES(\
        problem = SASphereProblem(dimensions = 25, accuracy = -4),
        mu = 15, 
        lambd = 100,
        pi = 50,
        theta = 0.7,
...
class TR25AllExperiment(Experiment):

    def __init__(self):
        self._file_call = 'evopy_experiments/tr25-all-rfc/experiment_calls.csv'
        self._file_fitnesses = 'evopy_experiments/tr25-all-rfc/experiment_fitnesses.csv'
        self._file_acc = 'evopy_experiments/tr25-all-rfc/experiment_acc.csv'
    ...
    def _write_stats(self, methodname, sample, stats):
        ...
        self._writer_calls.writerow(\
            ...
            0, #stats["train-function-calls"],
            stats["constraint-calls"],
            0, #stats["metamodel-calls"],
