...
accsfile = file("output/accuracies_file.save", "r")
cv_accuracies = load(accsfile)
...
for problem in cv_accuracies.keys():    
    ...
    plt.ylabel("Gemittelte empirische Vorhersagegenauigkeit")
    plt.xlim([0, 50])
    plt.ylim([0.0, 1.0])
    ...
    optimizers = cv_accuracies[problem].keys()
    for index, optimizer in enumerate(optimizers):
        cv_accuracies_po = cv_accuracies[problem][optimizer]
        cv_accuracies_agg, errors_agg =\
            TimeseriesAggregator(cv_accuracies_po).get_aggregate()

        generations = range(0, len(cv_accuracies_agg))
