...
for problem in problems:
    ...
    b = map(lambda x : x/(1.0), range(int(minimum) * 1, int(maximum) * 1 + 1, 1))
    pdfs1, bins1, patches1 = hist(x1_log, normed=True, alpha=0.5,\
        histtype='step', bins = b, edgecolor="g")
    ...
    b2 = map(lambda x : x/(1.0), range(int(minimum) * 1, int(maximum) * 1 + 1, 1))
    pdfs2, bins2, patches2 = hist(x2_log, normed=True, alpha=0.5,\
        histtype='step', bins = b2, edgecolor="#004779")
