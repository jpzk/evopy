...
from numpy import matrix, log10, log2, array
...
cfile = file("output/parameterc_file.save", "r")
parameter_c = load(cfile)
...
filter_none = lambda x : type(x) != type(None)

for problem in problems:
    for optimizer in optimizers[problem]:
        cs = parameter_c[problem][optimizer]
        cs = list(chain.from_iterable(cs))

        variables['min'][problem][optimizer] = log2(min(filter(filter_none, cs)))
        variables['max'][problem][optimizer] = log2(max(filter(filter_none, cs)))
